...
#define N_INPUTS 9 // darkreading + 7 leds, no color sensor for now
#define N_RESULTS 1
#define TENSOR_ARENA_SIZE 2 * 1024
#define N_OUTPUTS 8 // number of LEDs
#define RESET 2
#define ADDR 0x20
...
double allReadings[19]; // combined IR + color readings
...
void resultsTimeout(lv_timer_t *timer) {
    lv_timer_del(timer);
    results_timer = lv_timer_create(resultsTimeout, RESULTS_DISPLAY_MS, nullptr);
...
void runScan() {
    ...
    // Prepare input - do this AFTER all readings
    input[0] = (float)darkReading;
    for (int i = 0; i < N_OUTPUTS; i++) {
        input[i + 1] = (float)ledReadings[i];
    }
