...
static uint8_t setBrightness(int output, int percents)
{
    // Bounds checking without assert
    if (output < 0 || output >= N_OUTPUTS) {
        Serial.println("ERROR: Invalid output index");
        return 1;
    }
    if (percents < 0 || percents > 100) {
        Serial.println("ERROR: Invalid percents");
        return 1;
    }
    ...
    // Ensure Wire buffer is in a known state
    if (Wire.available()) {
    ...
    uint8_t writeResult = Wire.write(reg);
    if (writeResult != 1) {
        Serial.print("Wire.write(reg) failed: "); Serial.println(writeResult);
        Wire.endTransmission();
        return 2;
    }
    
    writeResult = Wire.write(pwm);
    if (writeResult != 1) {
        Serial.print("Wire.write(pwm) failed: "); Serial.println(writeResult);
        Wire.endTransmission();
        return 3;
    }
    ...
    if (result != 0) {
        Serial.print("Wire.endTransmission failed: "); Serial.println(result);
    }
    return result;
}
...
bool TLC59208::begin()
{
    ...
    return (ret == 0);  // Return true if successful
}
